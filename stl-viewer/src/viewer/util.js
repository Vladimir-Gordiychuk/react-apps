...
/**
 * Loads Binary STL file from specified resource
 * and transforms it to StlModel object.
 * @param {string} url
 * @returns {StlModel}
 */
export const loadBinaryStl = async (url) => {
    ...
    model.readArrayBuffer(response.data);
    return model;
};
...
export const loadTextResource = function (url, callback) {
    ...
    request.onload = function () {
        if (request.status < 200 || request.status > 299) {
            callback(
                "Error: HTTP status " + request.status + " on resource " + url
            );
        } else {
...
export const initGL = function (
    ...
    if (enableDepthTest) gl.enable(gl.DEPTH_TEST);
...
export const loadTexture = function (gl, image) {
    if (!image) throw new Error("Invalid image is used to create a texture.");
